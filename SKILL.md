...
5. Treat user-supplied title, brand, and allowed supporting text as authoritative. If no exact title or brand is supplied, derive one short, source-specific English main title from the photograph's subject, theme, mood, function, or symbolism; use that derived title as the current authority. Render only the current allowed strings; remove stale text from earlier variants, gibberish, and repeated identical lockups.
...
Save every accepted iteration under a new versioned filename. Before completion, fully decode the output and record its dimensions, upper and lower pixel heights, color mode, midpoint measurement, and whole-file SHA-256. For every lower-half-only revision, compare the candidate upper half with the approved upper half and record either zero differing pixels or an identical upper-half crop SHA-256. Inspect fidelity, text, carrier count, layout, palette, and the visible transition; a nominal 3:4 ratio or visual approximation is insufficient.
